...
function resolve(source, file, options) {
  ...
  if ((/\.jsx?$/.test(foundNodePath) || opts.alwaysTryTypes && !foundNodePath) && !/^@types[/\\]/.test(source) && !path__default["default"].isAbsolute(source) && !source.startsWith(".")) {
...
function getMappedPath(source, file, retry) {
  let paths = [];
  if (RELATIVE_PATH_PATTERN.test(source)) {
    ...
    paths = mappers.flatMap((mapper) => mapper == null ? void 0 : mapper(source)).filter(isFile);
  }
  if (retry && paths.length === 0) {
    if (JS_EXT_PATTERN.test(source)) {
      ...
      return getMappedPath(basename + tsExt, file) || getMappedPath(source + "/index.ts", file) || getMappedPath(source + "/index.tsx", file) || getMappedPath(source + "/index.js", file) || getMappedPath(basename + ".d" + (tsExt === ".tsx" ? ".ts" : tsExt), file, false);
    }
    return getMappedPath(source + ".ts", file) || getMappedPath(source + ".tsx", file) || getMappedPath(source + ".js", file) || getMappedPath(source + ".d.ts", file) || getMappedPath(source + "/index.ts", file) || getMappedPath(source + "/index.tsx", file) || getMappedPath(source + "/index.js", file);
  }
  if (paths.length > 1) {
